...
extract_MK(const std::vector<Tensor> &inputs) {
  ...
  if (inputs.at(0).shape.size() == 2) {
    M = inputs.at(input_idx).shape.at(1);
    K = inputs.at(output_idx).shape.at(1);
  }
...
AddTanhLPNorm<InT, WtT, OutT>::map_padded_shape(size_t M, size_t N) const {
...
template <typename InT, typename WtT, typename OutT>
std::once_flag AddTanhLPNorm<InT, WtT, OutT>::logger_flag_;

template <typename InT, typename WtT, typename OutT>
uint64_t AddTanhLPNorm<InT, WtT, OutT>::AddTanhLPNorm_count = 0;

template <typename InT, typename WtT, typename OutT>
std::once_flag AddTanhLPNorm<InT, WtT, OutT>::instr_reg_flag_;

template <typename InT, typename WtT, typename OutT>
void AddTanhLPNorm<InT, WtT, OutT>::debug(bool enable) {
  debug_ = enable;
}

template <typename InT, typename WtT, typename OutT>
std::string AddTanhLPNorm<InT, WtT, OutT>::get_instr_key(std::string prefix,
                                                         size_t m,
                                                         size_t k) const {
  // return prefix + "_" + std::to_string(m) + "_" + std::to_string(k) + ".bin";
  return "AddTanhLPNorm_" + prefix + "_" + std::to_string(m) + "_" +
         std::to_string(k);
}

template <typename InT, typename WtT, typename OutT>
const std::vector<uint8_t> AddTanhLPNorm<InT, WtT, OutT>::get_transaction_bin(
...
AddTanhLPNorm<InT, WtT, OutT>::get_super_kernel_params(
    std::vector<Tensor> &input, std::vector<Tensor> &output,
    const std::map<std::string, std::any> &attr) const {
  auto [M, K] = extract_MK(input);
  auto [Mo, Ko] = map_padded_shape(M, K);
...
template <typename InT, typename WtT, typename OutT>
std::vector<OpArgMap> AddTanhLPNorm<InT, WtT, OutT>::get_buffer_reqs(
    std::vector<Tensor> &input, std::vector<Tensor> &output,
    const std::map<std::string, std::any> &attr) const {
  ...
  size_t output_bo_size = (No * sizeof(OutT));
  size_t super_kernel_size = get_super_kernel_params(input, output).size();

  std::vector<OpArgMap> arg_map{
      {OpArgMap::OpArgType::INPUT, 1, 0, 0, input_bo_size},
      {OpArgMap::OpArgType::INPUT, 1, 1, input_bo_size, input_bo_size},
...
template <typename InT, typename WtT, typename OutT>
void AddTanhLPNorm<InT, WtT, OutT>::setup_instr_registry() {
...
template <typename InT, typename WtT, typename OutT>
AddTanhLPNorm<InT, WtT, OutT>::AddTanhLPNorm(
  ...
  default_shapes_["AddTanhLPNorm_4x2_abf16accbf16"] =
      std::vector<std::tuple<int, int>>();

  default_shapes_["AddTanhLPNorm_4x2_abf16accbf16"].push_back(
      std::make_tuple(768, 128));

  // raw shape is the actual shape from ONNX, sequence needs to match with
  // default_shape
  raw_shapes_["AddTanhLPNorm_4x2_abf16accbf16"] =
      std::vector<std::tuple<int, int>>();

  raw_shapes_["AddTanhLPNorm_4x2_abf16accbf16"].push_back(
      std::make_tuple(768, 100));
  ...
  AddTanhLPNorm_id_ = AddTanhLPNorm_count++;
  ...
  txn_fname_prefix_ = "AddTanhLPNorm_4x2_" + txnbin_a_header.at(a_dtype_) +
                      txnbin_acc_header.at(c_dtype_);

  param_fname_prefix_ = "AddTanhLPNorm_4x2_" + txnbin_a_header.at(a_dtype_) +
                        txnbin_acc_header.at(c_dtype_);
  ...
  std::call_once(logger_flag_, []() {
    std::string header =
        "AddTanhLPNorm_id M K N kernel_m kernel_k kernel_n Execute"
  ...
  RYZENAI_LOG_TRACE("[AddTanhLPNorm] ID: " + std::to_string(AddTanhLPNorm_id_) +
                    ", XCLBIN: " + XCLBIN_FNAME +
...
template <typename InT, typename WtT, typename OutT>
void AddTanhLPNorm<InT, WtT, OutT>::set_params(
...
template <typename InT, typename WtT, typename OutT>
void AddTanhLPNorm<InT, WtT, OutT>::initialize_const_params(
    ConstBufferIO &io, const std::vector<Tensor> &const_params,
    const std::map<std::string, std::any> &attr) {
  RYZENAI_LOG_TRACE("AddTanhLPNorm initialize_const_params(ptr) ...");

  DD_THROW_IF(
      (const_params.size() != 1) || (const_params.at(0).shape.size() != 1),
      OpsFusion::dd_format("Unsupported const spec for AddTanhLPNorm\n") +
          OpsFusion::dd_format(
  ...
  RYZENAI_LOG_TRACE("AddTanhLPNorm initialize_const_params(ptr) ... DONE");
}
...
void AddTanhLPNorm<InT, WtT, OutT>::initialize_const_params(
  ...
  if (const_params.size() != 1) {
    throw std::runtime_error(
        "AddTanhLPNorm IPU Wrapper expect to have one constant.");
  }
  ...
  RYZENAI_LOG_TRACE("AddTanhLPNorm: A_BO_SIZE:" + std::to_string(A_BO_SIZE) +
...
template <typename InT, typename WtT, typename OutT>
void AddTanhLPNorm<InT, WtT, OutT>::execute(std::vector<Tensor> &input,
                                            std::vector<Tensor> &output) {
  // Check the number of inputs
  if (input.size() != 2) {
    throw std::runtime_error(
        "AddTanhLPNorm IPU Wrapper expect to have two inputs.");
  }
  const int a_idx = 0;
  ...
  auto [M, K] = map_padded_shape(input.at(0).shape[1], output.at(0).shape[1]);
  ...
  memcpy((void *)(reinterpret_cast<int8_t *>(a_bo_map) + M * sizeof(InT)),
         (void *)b, a_size);
  ...
  uint32_t instr_bo_words =
      static_cast<uint32_t>(instr_bo.size() / sizeof(int));
  ...
  RYZENAI_LOG_INFO(
      std::to_string(AddTanhLPNorm_id_) + " " + std::to_string(a_shape_[0]) +
...
template class AddTanhLPNorm<uint16_t, uint16_t, uint16_t>;
} // namespace ryzenai
