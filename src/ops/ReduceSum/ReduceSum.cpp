...
ReduceSum<InT, WtT, OutT>::map_padded_shape(size_t M, size_t N) const {
...
template <typename InT, typename WtT, typename OutT>
std::once_flag ReduceSum<InT, WtT, OutT>::logger_flag_;

template <typename InT, typename WtT, typename OutT>
uint64_t ReduceSum<InT, WtT, OutT>::ReduceSum_count = 0;

template <typename InT, typename WtT, typename OutT>
std::once_flag ReduceSum<InT, WtT, OutT>::instr_reg_flag_;

template <typename InT, typename WtT, typename OutT>
void ReduceSum<InT, WtT, OutT>::debug(bool enable) {
  debug_ = enable;
}

template <typename InT, typename WtT, typename OutT>
std::string ReduceSum<InT, WtT, OutT>::get_instr_key(std::string prefix,
                                                     size_t m, size_t k) const {
  // return prefix + "_" + std::to_string(m) + "_" + std::to_string(k) + ".bin";
  return "ReduceSum_" + prefix + "_" + std::to_string(m) + "_" +
         std::to_string(k);
}

template <typename InT, typename WtT, typename OutT>
const std::vector<uint8_t> ReduceSum<InT, WtT, OutT>::get_transaction_bin(
...
template <typename InT, typename WtT, typename OutT>
const std::vector<uint8_t> ReduceSum<InT, WtT, OutT>::get_super_kernel_params(
    std::vector<Tensor> &input, std::vector<Tensor> &output,
    const std::map<std::string, std::any> &attr) const {
  auto [M, K] = extract_MK(input);
  auto [Mo, Ko] = map_padded_shape(M, K);
...
template <typename InT, typename WtT, typename OutT>
std::vector<OpArgMap> ReduceSum<InT, WtT, OutT>::get_buffer_reqs(
    std::vector<Tensor> &input, std::vector<Tensor> &output,
    const std::map<std::string, std::any> &attr) const {
  ...
  size_t const_params_bo_size = 16; // dummy
...
template <typename InT, typename WtT, typename OutT>
void ReduceSum<InT, WtT, OutT>::setup_instr_registry() {
...
template <typename InT, typename WtT, typename OutT>
ReduceSum<InT, WtT, OutT>::ReduceSum(
  ...
  default_shapes_["ReduceSum_4x2_abf16accbf16"] =
      std::vector<std::tuple<int, int>>();

  default_shapes_["ReduceSum_4x2_abf16accbf16"].push_back(
      std::make_tuple(512, 768));

  // raw shape is the actual shape from ONNX, sequence needs to match with
  // default_shape
  raw_shapes_["ReduceSum_4x2_abf16accbf16"] =
      std::vector<std::tuple<int, int>>();

  raw_shapes_["ReduceSum_4x2_abf16accbf16"].push_back(
      std::make_tuple(512, 768));
  ...
  ReduceSum_id_ = ReduceSum_count++;
  /*select xclbin based on the input/output types*/
  std::string XCLBIN_FNAME = OpInterface::get_dd_base_dir() +
                             ryzenai::START_TAIL_4x2_MS_SHELL_QDQ_XCLBIN_PATH;

  txn_fname_prefix_ = "ReduceSum_4x2_" + txnbin_a_header.at(a_dtype_) +
                      txnbin_acc_header.at(c_dtype_);

  param_fname_prefix_ = "ReduceSum_4x2_" + txnbin_a_header.at(a_dtype_) +
                        txnbin_acc_header.at(c_dtype_);
  ...
  std::call_once(logger_flag_, []() {
    std::string header = "ReduceSum_id M K N kernel_m kernel_k kernel_n Execute"
  ...
  RYZENAI_LOG_TRACE("[ReduceSum] ID: " + std::to_string(ReduceSum_id_) +
                    ", XCLBIN: " + XCLBIN_FNAME +
...
template <typename InT, typename WtT, typename OutT>
void ReduceSum<InT, WtT, OutT>::set_params(const std::string &model_name,
...
template <typename InT, typename WtT, typename OutT>
void ReduceSum<InT, WtT, OutT>::initialize_const_params(
    ConstBufferIO &io, const std::vector<Tensor> &const_params,
    const std::map<std::string, std::any> &attr) {
  RYZENAI_LOG_TRACE("ReduceSum initialize_const_params(ptr) ...");

  RYZENAI_LOG_TRACE("ReduceSum initialize_const_params(ptr) ... DONE");
}

template <typename InT, typename WtT, typename OutT>
void ReduceSum<InT, WtT, OutT>::execute(std::vector<Tensor> &input,
                                        std::vector<Tensor> &output) {
  // Check the number of inputs
  if (input.size() != 1) {
    throw std::runtime_error("ReduceSum IPU Wrapper expect to have one input.");
  }
  const int a_idx = 0;
  const int c_idx = 0;
  // The first data is a
  ...
  const size_t C_BO_SIZE = (1 * kernel_x_shape_[1] * c_dtype_size_);
  ...
  RYZENAI_LOG_TRACE("ReduceSum: a_size:" + std::to_string(a_size));
  ...
  RYZENAI_LOG_INFO(
      std::to_string(ReduceSum_id_) + " " + std::to_string(a_shape_[0]) + " " +
...
template class ReduceSum<uint16_t, uint16_t, uint16_t>;
} // namespace ryzenai
