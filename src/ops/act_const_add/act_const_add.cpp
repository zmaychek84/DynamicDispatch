...
act_const_add<InT, WtT, OutT>::map_padded_shape(size_t M, size_t N) const {
...
template <typename InT, typename WtT, typename OutT>
std::once_flag act_const_add<InT, WtT, OutT>::logger_flag_;

template <typename InT, typename WtT, typename OutT>
uint64_t act_const_add<InT, WtT, OutT>::act_const_add_count = 0;

template <typename InT, typename WtT, typename OutT>
std::once_flag act_const_add<InT, WtT, OutT>::instr_reg_flag_;

template <typename InT, typename WtT, typename OutT>
void act_const_add<InT, WtT, OutT>::debug(bool enable) {
  debug_ = enable;
}

template <typename InT, typename WtT, typename OutT>
std::string act_const_add<InT, WtT, OutT>::get_instr_key(std::string prefix,
                                                         size_t m,
                                                         size_t k) const {
  // return prefix + "_" + std::to_string(m) + "_" + std::to_string(k) + ".bin";
  return "act_const_add_" + prefix + "_" + std::to_string(m) + "_" +
         std::to_string(k);
}

template <typename InT, typename WtT, typename OutT>
const std::vector<uint8_t> act_const_add<InT, WtT, OutT>::get_transaction_bin(
...
act_const_add<InT, WtT, OutT>::get_super_kernel_params(
    std::vector<Tensor> &input, std::vector<Tensor> &output,
    const std::map<std::string, std::any> &attr) const {
  auto [M, K] = extract_MK(input);
  auto [Mo, Ko] = map_padded_shape(M, K);
...
template <typename InT, typename WtT, typename OutT>
std::vector<OpArgMap> act_const_add<InT, WtT, OutT>::get_buffer_reqs(
    std::vector<Tensor> &input, std::vector<Tensor> &output,
    const std::map<std::string, std::any> &attr) const {
  ...
  size_t const_params_bo_size = (Mo * No * sizeof(InT));
...
template <typename InT, typename WtT, typename OutT>
void act_const_add<InT, WtT, OutT>::setup_instr_registry() {
...
template <typename InT, typename WtT, typename OutT>
act_const_add<InT, WtT, OutT>::act_const_add(
    const std::string &a_dtype, const std::string &b_dtype,
    const std::string &c_dtype, bool load_xrt,
    const std::map<std::string, std::any> &attr) {

  txnbin_a_header = {{"bfloat16", "abf16"}, {"uint16", "a16"}};
  txnbin_b_header = {{"bfloat16", "wbf16"}, {"uint16", "w16"}};
  txnbin_acc_header = {{"bfloat16", "accbf16"}, {"uint16", "acc16"}};

  // default shape is the padded shaped used in AIE for BO allocation
  default_shapes_["act_const_add_4x2_abf16wbf16accbf16"] =
      std::vector<std::tuple<int, int>>();

  default_shapes_["act_const_add_4x2_abf16wbf16accbf16"].push_back(
  ...
  // default_shape
  raw_shapes_["act_const_add_4x2_abf16wbf16accbf16"] =
      std::vector<std::tuple<int, int>>();

  raw_shapes_["act_const_add_4x2_abf16wbf16accbf16"].push_back(
      std::make_tuple(77, 768));
  raw_shapes_["act_const_add_4x2_abf16wbf16accbf16"].push_back(
  ...
  act_const_add_id_ = act_const_add_count++;
  ...
  txn_fname_prefix_ = "act_const_add_4x2_" + txnbin_a_header.at(a_dtype_) +
                      txnbin_b_header.at(b_dtype_) +
                      txnbin_acc_header.at(c_dtype_);

  param_fname_prefix_ = "act_const_add_4x2_" + txnbin_a_header.at(a_dtype_) +
                        txnbin_b_header.at(b_dtype_) +
                        txnbin_acc_header.at(c_dtype_);
  ...
  std::call_once(logger_flag_, []() {
    std::string header =
        "act_const_add_id M K N kernel_m kernel_k kernel_n Execute"
  ...
  RYZENAI_LOG_TRACE("[act_const_add] ID: " + std::to_string(act_const_add_id_) +
                    ", XCLBIN: " + XCLBIN_FNAME +
...
template <typename InT, typename WtT, typename OutT>
void act_const_add<InT, WtT, OutT>::set_params(
    const std::string &model_name, std::vector<size_t> input_shape) {

  std::string XCLBIN_FNAME = OpInterface::get_dd_base_dir() +
                             ryzenai::START_TAIL_4x2_MS_SHELL_QDQ_XCLBIN_PATH;

  if (model_name == "mtea0a") {
    XCLBIN_FNAME =
        OpInterface::get_dd_base_dir() + ryzenai::mtea0a_A8W8_QDQ_XCLBIN_PATH;
  }

  // for memory allocation
  auto [M, N] = map_padded_shape(input_shape.at(1), input_shape.at(2));
  a_shape_[0] = M;
  a_shape_[1] = N;
...
template <typename InT, typename WtT, typename OutT>
void act_const_add<InT, WtT, OutT>::initialize_const_params(
    ConstBufferIO &io, const std::vector<Tensor> &const_params,
    const std::map<std::string, std::any> &attr) {
  RYZENAI_LOG_TRACE("act_const_add initialize_const_params(ptr) ...");

  DD_THROW_IF(
      (const_params.size() != 1) || (const_params.at(0).shape.size() != 3),
      OpsFusion::dd_format("Unsupported const spec for act_const_add\n") +
          OpsFusion::dd_format(
  ...
  auto w_size = const_params.at(w_idx).shape[1] *
                const_params.at(w_idx).shape[2] * sizeof(uint16_t);

  io.write(0, w, w_size);

  RYZENAI_LOG_TRACE("act_const_add initialize_const_params(ptr) ... DONE");
}
...
void act_const_add<InT, WtT, OutT>::initialize_const_params(
  ...
  if (const_params.size() != 1) {
    throw std::runtime_error("act_const_add IPU Wrapper expect to have "
                             "one constant (weights).");
  }
  ...
  RYZENAI_LOG_TRACE("act_const_add: A_BO_SIZE:" + std::to_string(A_BO_SIZE) +
...
template <typename InT, typename WtT, typename OutT>
void act_const_add<InT, WtT, OutT>::execute(std::vector<Tensor> &input,
                                            std::vector<Tensor> &output) {
  // Check the number of inputs
  if (input.size() != 1) {
    throw std::runtime_error(
        "act_const_add IPU Wrapper expect to have one input.");
  }
  const int a_idx = 0;
  ...
  a_shape_[0] = input.at(a_idx).shape.at(1);
  a_shape_[1] = input.at(a_idx).shape.at(2);
  ...
  RYZENAI_LOG_INFO(
      std::to_string(act_const_add_id_) + " " + std::to_string(a_shape_[0]) +
...
template class act_const_add<uint16_t, uint16_t, uint16_t>;
} // namespace ryzenai
