...
std::once_flag qdq_add<InT, WtT, OutT>::logger_flag_;

template <typename InT, typename WtT, typename OutT>
uint64_t qdq_add<InT, WtT, OutT>::qdq_add_count = 0;

template <typename InT, typename WtT, typename OutT>
std::once_flag qdq_add<InT, WtT, OutT>::instr_reg_flag_;

template <typename InT, typename WtT, typename OutT>
void qdq_add<InT, WtT, OutT>::debug(bool enable) {
  debug_ = enable;
}

template <typename InT, typename WtT, typename OutT>
std::tuple<size_t, size_t>
qdq_add<InT, WtT, OutT>::map_padded_shape(size_t M, size_t N) const {
...
template <typename InT, typename WtT, typename OutT>
std::string qdq_add<InT, WtT, OutT>::get_instr_key(std::string prefix, size_t m,
                                                   size_t k) const {
  return "qdqadd_" + prefix + "_" + std::to_string(m) + "_" + std::to_string(k);
}

template <typename InT, typename WtT, typename OutT>
void qdq_add<InT, WtT, OutT>::setup_instr_registry() {
...
template <typename InT, typename WtT, typename OutT>
qdq_add<InT, WtT, OutT>::qdq_add(const std::string &a_dtype,
  ...
  default_shapes_["qdqadd_4x4_a16a16acc16"].push_back(std::make_tuple(64, 768));

  // raw shape is the actual shape from ONNX
  raw_shapes_["qdqadd_4x4_a16a16acc16"] = std::vector<std::tuple<int, int>>();
  raw_shapes_["qdqadd_4x4_a16a16acc16"].push_back(std::make_tuple(64, 768));
  ...
  txn_fname_prefix_ = "qdqadd_4x4_" + txnbin_a_header.at(a_dtype_) +
                      txnbin_b_header.at(b_dtype_) +
                      txnbin_c_header.at(c_dtype_);

  param_fname_prefix_ = "qdqadd_4x4_" + txnbin_a_header.at(a_dtype_) +
                        txnbin_b_header.at(b_dtype_) +
                        txnbin_c_header.at(c_dtype_);
  ...
  std::call_once(logger_flag_, []() {
    std::string header = "qdq_add_id M K N kernel_m kernel_k kernel_n Execute"
  ...
  RYZENAI_LOG_TRACE("[ADD] ID: " + std::to_string(qdq_add_id_) + ", XCLBIN: " +
                    XCLBIN_FNAME + ", (a_dtype, b_dtype, c_dtype): (" +
                    a_dtype_ + ", " + b_dtype_ + ", " + c_dtype_ + ")");
}

template <typename InT, typename WtT, typename OutT>
void qdq_add<InT, WtT, OutT>::set_params(const std::string &model_name,
...
template <typename InT, typename WtT, typename OutT>
void qdq_add<InT, WtT, OutT>::initialize_const_params(
  ...
  RYZENAI_LOG_TRACE("qdqadd initialize_const_params(ptr) ... DONE");
}
...
void qdq_add<InT, WtT, OutT>::initialize_const_params(
  ...
  if (const_params.size() != 1) {
    throw std::runtime_error("qdqadd IPU Wrapper expect to have one constant.");
  }
  ...
  RYZENAI_LOG_TRACE("qdqadd: A_BO_SIZE:" + std::to_string(A_BO_SIZE) +
  ...
  if (is_ctrl_pkt_) {
    ...
    if (is_ctrl_pkt_) {
      ...
      RYZENAI_LOG_TRACE("matmulgeluadd patch ctrlpkt ... START");
      ...
      RYZENAI_LOG_TRACE("qdqadd patch ctrlpkt ... DONE");
    }
  }
}

template <typename InT, typename WtT, typename OutT>
void qdq_add<InT, WtT, OutT>::execute(std::vector<Tensor> &input,
                                      std::vector<Tensor> &output) {
  // Check the number of inputs
  if (input.size() != 2) {
    throw std::runtime_error("qdqadd IPU Wrapper expect to have two inputs.");
  }
  const int a_idx = 0;
  ...
  RYZENAI_LOG_TRACE("qdqadd: a_size:" + std::to_string(a_size));
  // a_bo copy
  ...
  RYZENAI_LOG_INFO(
      std::to_string(qdq_add_id_) + " " + std::to_string(a_shape_[0]) + " " +
...
template <typename InT, typename WtT, typename OutT>
const std::vector<uint8_t> qdq_add<InT, WtT, OutT>::get_transaction_bin(
...
template <typename InT, typename WtT, typename OutT>
const std::vector<uint8_t> qdq_add<InT, WtT, OutT>::get_super_kernel_params(
    std::vector<Tensor> &input, std::vector<Tensor> &output,
    const std::map<std::string, std::any> &attr) const {
  auto [M, K] = extract_MK(input);
  auto [Mo, Ko] = map_padded_shape(M, K);
...
template <typename InT, typename WtT, typename OutT>
std::vector<uint8_t> qdq_add<InT, WtT, OutT>::get_ctrl_pkts(
    std::vector<Tensor> &input, std::vector<Tensor> &output,
    const std::map<std::string, std::any> &attr) const {
  auto [M, K] = extract_MK(input);
  auto [Mo, Ko] = map_padded_shape(M, K);
...
template <typename InT, typename WtT, typename OutT>
std::vector<CtrlPktPatchInfo> qdq_add<InT, WtT, OutT>::get_ctrl_pkt_patch_info(
    std::vector<Tensor> &input, std::vector<Tensor> &output,
    const std::map<std::string, std::any> &attr) const {
  auto [M, K] = extract_MK(input);
  auto [Mo, Ko] = map_padded_shape(M, K);
  ...
  try {
    ...
    // throw std::runtime_error("qdqadd : Can not file the ctrl_meta.json
    // file");
    return {};
  }
}
...
template <typename InT, typename WtT, typename OutT>
std::vector<OpArgMap> qdq_add<InT, WtT, OutT>::get_buffer_reqs(
    std::vector<Tensor> &input, std::vector<Tensor> &output,
    const std::map<std::string, std::any> &attr) const {
...
template class qdq_add<uint8_t, uint8_t, uint16_t>;
template class qdq_add<uint16_t, uint16_t, uint16_t>;
