...
std::once_flag elwdiv_qdq<InT, WtT, OutT>::logger_flag_;

template <typename InT, typename WtT, typename OutT>
uint64_t elwdiv_qdq<InT, WtT, OutT>::elwdiv_qdq_count = 0;

template <typename InT, typename WtT, typename OutT>
std::once_flag elwdiv_qdq<InT, WtT, OutT>::instr_reg_flag_;

template <typename InT, typename WtT, typename OutT>
void elwdiv_qdq<InT, WtT, OutT>::debug(bool enable) {
  debug_ = enable;
}

template <typename InT, typename WtT, typename OutT>
std::tuple<size_t, size_t>
elwdiv_qdq<InT, WtT, OutT>::map_padded_shape(size_t M, size_t N) const {
...
template <typename InT, typename WtT, typename OutT>
std::string elwdiv_qdq<InT, WtT, OutT>::get_instr_key(std::string prefix,
                                                      size_t m,
                                                      size_t k) const {
  return "elwdiv_qdq_" + prefix + "_" + std::to_string(m) + "_" +
         std::to_string(k);
}

template <typename InT, typename WtT, typename OutT>
void elwdiv_qdq<InT, WtT, OutT>::setup_instr_registry() {
...
template <typename InT, typename WtT, typename OutT>
elwdiv_qdq<InT, WtT, OutT>::elwdiv_qdq(
  ...
  default_shapes_["elwdiv_qdq_4x2_abf16abf16accbf16"] =
      std::vector<std::tuple<int, int>>();
  default_shapes_["elwdiv_qdq_4x2_abf16abf16acc16"] =
      std::vector<std::tuple<int, int>>();

  default_shapes_["elwdiv_qdq_4x2_abf16abf16accbf16"].push_back(
      std::make_tuple(1, 768));
  default_shapes_["elwdiv_qdq_4x2_abf16abf16acc16"].push_back(
      std::make_tuple(1, 768));

  // raw shape is the actual shape from ONNX
  raw_shapes_["elwdiv_qdq_4x2_abf16abf16accbf16"] =
      std::vector<std::tuple<int, int>>();
  raw_shapes_["elwdiv_qdq_4x2_abf16abf16acc16"] =
      std::vector<std::tuple<int, int>>();

  raw_shapes_["elwdiv_qdq_4x2_abf16abf16accbf16"].push_back(
      std::make_tuple(1, 768));
  raw_shapes_["elwdiv_qdq_4x2_abf16abf16acc16"].push_back(
      std::make_tuple(1, 768));
  ...
  elwdiv_qdq_id_ = elwdiv_qdq_count++;
  ...
  txn_fname_prefix_ = "elwdiv_qdq_4x2_" + txnbin_a_header.at(a_dtype_) +
                      txnbin_b_header.at(b_dtype_) +
                      txnbin_c_header.at(c_dtype_);

  param_fname_prefix_ = "elwdiv_qdq_4x2_" + txnbin_a_header.at(a_dtype_) +
                        txnbin_b_header.at(b_dtype_) +
                        txnbin_c_header.at(c_dtype_);
  ...
  std::call_once(logger_flag_, []() {
    std::string header =
        "elwdiv_qdq_id M K N kernel_m kernel_k kernel_n Execute"
  ...
  RYZENAI_LOG_TRACE("[elwdiv_qdq] ID: " + std::to_string(elwdiv_qdq_id_) +
                    ", XCLBIN: " + XCLBIN_FNAME +
...
template <typename InT, typename WtT, typename OutT>
void elwdiv_qdq<InT, WtT, OutT>::set_params(const std::string &model_name,
...
template <typename InT, typename WtT, typename OutT>
void elwdiv_qdq<InT, WtT, OutT>::initialize_const_params(
    ConstBufferIO &io, const std::vector<Tensor> &const_params,
    const std::map<std::string, std::any> &attr) {
  RYZENAI_LOG_TRACE("elwdiv_qdq initialize_const_params(ptr) ...");
  ...
  RYZENAI_LOG_TRACE("elwdiv_qdq initialize_const_params(ptr) ... DONE");
}
...
void elwdiv_qdq<InT, WtT, OutT>::initialize_const_params(
  ...
  RYZENAI_LOG_TRACE("elwdiv_qdq: A_BO_SIZE:" + std::to_string(A_BO_SIZE) +
...
template <typename InT, typename WtT, typename OutT>
void elwdiv_qdq<InT, WtT, OutT>::execute(std::vector<Tensor> &input,
                                         std::vector<Tensor> &output) {
  // Check the number of inputs
  if (input.size() != 2) {
    throw std::runtime_error(
        "elwdiv_qdq IPU Wrapper expect to have two inputs.");
  }
  const int a_idx = 0;
  ...
  RYZENAI_LOG_TRACE("elwdiv_qdq: a_size:" + std::to_string(a_size));
  // a_bo copy
  ...
  RYZENAI_LOG_INFO(
      std::to_string(elwdiv_qdq_id_) + " " + std::to_string(a_shape_[0]) + " " +
...
template <typename InT, typename WtT, typename OutT>
const std::vector<uint8_t> elwdiv_qdq<InT, WtT, OutT>::get_transaction_bin(
...
template <typename InT, typename WtT, typename OutT>
const std::vector<uint8_t> elwdiv_qdq<InT, WtT, OutT>::get_super_kernel_params(
    std::vector<Tensor> &input, std::vector<Tensor> &output,
    const std::map<std::string, std::any> &attr) const {
  auto [M, K] = extract_MK(input);
  auto [Mo, Ko] = map_padded_shape(M, K);
...
template <typename InT, typename WtT, typename OutT>
std::vector<OpArgMap> elwdiv_qdq<InT, WtT, OutT>::get_buffer_reqs(
    std::vector<Tensor> &input, std::vector<Tensor> &output,
    const std::map<std::string, std::any> &attr) const {
...
template class elwdiv_qdq<uint16_t, uint16_t, uint16_t>;
