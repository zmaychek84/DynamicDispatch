...
layernorm<InT, WtT, OutT>::map_padded_shape(size_t M, size_t N) const {
...
layernorm<InT, WtT, OutT>::layernorm(
  ...
  default_shapes_["lrn_4x2_abf16acc8"].push_back(std::make_tuple(256, 768));
  ...
  raw_shapes_["lrn_4x2_abf16acc8"].push_back(std::make_tuple(256, 768));

  raw_shapes_["lrn_4x2_abf16acc16"].push_back(std::make_tuple(77, 768));
...
void layernorm<InT, WtT, OutT>::initialize_const_params(
  ...
  std::vector<size_t> shape = const_params.at(gamma_idx).shape;
  size_t size_msk = shape[0] * shape[1] * b_dtype_size_;

  shape = const_params.at(beta_idx).shape;
  size_msk += shape[0] * shape[1] * b_dtype_size_;
