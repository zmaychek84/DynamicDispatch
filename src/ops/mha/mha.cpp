...
template <typename InT, typename WtT, typename OutT>
std::string mha<InT, WtT, OutT>::get_instr_key(std::string prefix,
                                               std::vector<size_t> &mat) const {
  std::string out_str = "mha_" + prefix;
...
mha<InT, WtT, OutT>::mha(const std::string &a_dtype, const std::string &b_dtype,
  ...
  default_shapes_["mha_a16w8acc16"] = std::vector<std::vector<size_t>>();
  default_shapes_["mha_a16w8acc16"].push_back(std::vector<size_t>{3, 77, 1024});
  ...
  txn_fname_prefix_ = "mha_" + txnbin_a_header.at(a_dtype_) +
                      txnbin_b_header.at(b_dtype_) +
                      txnbin_acc_header.at(c_dtype_);

  param_fname_prefix_ = "mha_" + txnbin_a_header.at(a_dtype_) +
                        txnbin_b_header.at(b_dtype_) +
                        txnbin_acc_header.at(c_dtype_);
  ...
  RYZENAI_LOG_TRACE("[mha] ID: " + std::to_string(mha_id_) + ", XCLBIN: " +
                    XCLBIN_FNAME + ", (a_dtype, b_dtype, c_dtype): (" +
                    a_dtype + ", " + b_dtype + ", " + c_dtype + ")");
}

template <typename InT, typename WtT, typename OutT>
void mha<InT, WtT, OutT>::set_params(const std::string &model_name,
                                     std::vector<size_t> input_shape) {
  std::string XCLBIN_FNAME;

  if (model_name == "m7h4xjg") {
    XCLBIN_FNAME =
...
void mha<InT, WtT, OutT>::initialize_const_params(
  ...
  RYZENAI_LOG_TRACE("mha initialize_const_params(ptr) ...");
  DD_THROW_IF(
      (const_params.size() != 1) || (const_params.at(0).shape.size() != 2),
      OpsFusion::dd_format(
          "Unsupported const spec for mha\n"
  ...
  qdq_param[(16 * 2) + qdq_Mv_idx] = mha_m7h4xjg_st_pad;
  qdq_param[(16 * 2) + qdq_Nv_idx] = mha_m7h4xjg_st_pad;

  qdq_param[(16 * 3) + qdq_Mv_idx] = mha_m7h4xjg_st_pad;
  qdq_param[(16 * 3) + qdq_Nv_idx] = mha_m7h4xjg_val_subv_cols;

  io.write(0, (void *)qdq_param, size_qdqparam);

  RYZENAI_LOG_TRACE("mha initialize_const_params(ptr) ... DONE");
}
...
void mha<InT, WtT, OutT>::initialize_const_params(
  ...
  DD_ASSERT((const_params.size() == 1),
            OpsFusion::dd_format("mha expects one constant. Got {}",
                                 const_params.size()));
...
void mha<InT, WtT, OutT>::execute(std::vector<Tensor> &input,
  ...
  // Check the number of inputs
  if (input.size() != 3) {
    throw std::runtime_error("mha IPU Wrapper expect to have one input.");
  }
  ...
  K = input.at(q_idx).shape.at(0);
  N = input.at(q_idx).shape.at(1);
  ...
  std::vector<size_t> param_shape = {3, K, N};
  ...
  RYZENAI_LOG_INFO(
      std::to_string(mha_id_) + " " + std::to_string(a_shape_[0]) + " " +
...
const std::vector<uint8_t> mha<InT, WtT, OutT>::get_transaction_bin(
  ...
  std::vector<size_t> param_shape = {3, Q_shape[0], Q_shape[1]};
...
template <typename InT, typename WtT, typename OutT>
const std::vector<uint8_t> mha<InT, WtT, OutT>::get_super_kernel_params(
    std::vector<Tensor> &input, std::vector<Tensor> &output,
    const std::map<std::string, std::any> &attr) const {
  auto Q_shape = extract_shape(input.at(0));
...
std::vector<OpArgMap> mha<InT, WtT, OutT>::get_buffer_reqs(
  ...
  auto out_shape = extract_shape(output.at(0));
...
template class mha<uint16_t, uint8_t, uint16_t>;
} // namespace ryzenai
