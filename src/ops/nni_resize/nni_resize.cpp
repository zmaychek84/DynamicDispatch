...
nni_resize<InT, OutT>::map_padded_shape(size_t H, size_t W, size_t C) const {
  ...
  for (int i = 0; i < supported_shapes.size(); i++) {
    ...
    if (H == std::get<0>(mat) && W == std::get<1>(mat) &&
        C == std::get<2>(mat)) {
      fidx = i;
      f_found = true;
      break;
...
template <typename InT, typename OutT>
std::string nni_resize<InT, OutT>::get_instr_key(std::string prefix, size_t h,
