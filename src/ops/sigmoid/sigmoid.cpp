...
sigmoid<InT, WtT, OutT>::map_padded_shape(size_t M, size_t N) const {
...
template <typename InT, typename WtT, typename OutT>
std::once_flag sigmoid<InT, WtT, OutT>::logger_flag_;

template <typename InT, typename WtT, typename OutT>
uint64_t sigmoid<InT, WtT, OutT>::sigmoid_count = 0;

template <typename InT, typename WtT, typename OutT>
std::once_flag sigmoid<InT, WtT, OutT>::instr_reg_flag_;

template <typename InT, typename WtT, typename OutT>
void sigmoid<InT, WtT, OutT>::debug(bool enable) {
  debug_ = enable;
}

template <typename InT, typename WtT, typename OutT>
std::string sigmoid<InT, WtT, OutT>::get_instr_key(std::string prefix, size_t m,
                                                   size_t k) const {
  // return prefix + "_" + std::to_string(m) + "_" + std::to_string(k) + ".bin";
  return "sigmoid_" + prefix + "_" + std::to_string(m) + "_" +
         std::to_string(k);
}

template <typename InT, typename WtT, typename OutT>
const std::vector<uint8_t> sigmoid<InT, WtT, OutT>::get_transaction_bin(
...
template <typename InT, typename WtT, typename OutT>
const std::vector<uint8_t> sigmoid<InT, WtT, OutT>::get_super_kernel_params(
    std::vector<Tensor> &input, std::vector<Tensor> &output,
    const std::map<std::string, std::any> &attr) const {
  auto [M, K] = extract_MK(input);
  auto [Mo, Ko] = map_padded_shape(M, K);
...
template <typename InT, typename WtT, typename OutT>
std::vector<OpArgMap> sigmoid<InT, WtT, OutT>::get_buffer_reqs(
    std::vector<Tensor> &input, std::vector<Tensor> &output,
    const std::map<std::string, std::any> &attr) const {
...
template <typename InT, typename WtT, typename OutT>
void sigmoid<InT, WtT, OutT>::setup_instr_registry() {
...
template <typename InT, typename WtT, typename OutT>
sigmoid<InT, WtT, OutT>::sigmoid(const std::string &a_dtype,
  ...
  default_shapes_["sigmoid_4x2_abf16accbf16"] =
      std::vector<std::tuple<int, int>>();
  default_shapes_["sigmoid_4x2_a16accbf16"] =
      std::vector<std::tuple<int, int>>();

  default_shapes_["sigmoid_4x2_abf16accbf16"].push_back(
      std::make_tuple(1, 512));
  default_shapes_["sigmoid_4x2_a16accbf16"].push_back(std::make_tuple(1, 512));

  // raw shape is the actual shape from ONNX, sequence needs to match with
  // default_shape
  raw_shapes_["sigmoid_4x2_abf16accbf16"] = std::vector<std::tuple<int, int>>();
  raw_shapes_["sigmoid_4x2_a16accbf16"] = std::vector<std::tuple<int, int>>();

  raw_shapes_["sigmoid_4x2_abf16accbf16"].push_back(std::make_tuple(1, 512));
  raw_shapes_["sigmoid_4x2_a16accbf16"].push_back(std::make_tuple(1, 512));
  ...
  sigmoid_id_ = sigmoid_count++;
  /*select xclbin based on the input/output types*/
  std::string XCLBIN_FNAME = OpInterface::get_dd_base_dir() +
                             ryzenai::START_TAIL_4x2_MS_SHELL_QDQ_XCLBIN_PATH;

  txn_fname_prefix_ = "sigmoid_4x2_" + txnbin_a_header.at(a_dtype_) +
                      txnbin_acc_header.at(c_dtype_);

  param_fname_prefix_ = "sigmoid_4x2_" + txnbin_a_header.at(a_dtype_) +
                        txnbin_acc_header.at(c_dtype_);
  ...
  std::call_once(logger_flag_, []() {
    std::string header = "sigmoid_id M K N kernel_m kernel_k kernel_n Execute"
  ...
  RYZENAI_LOG_TRACE("[sigmoid] ID: " + std::to_string(sigmoid_id_) +
                    ", XCLBIN: " + XCLBIN_FNAME +
...
template <typename InT, typename WtT, typename OutT>
void sigmoid<InT, WtT, OutT>::set_params(const std::string &model_name,
...
template <typename InT, typename WtT, typename OutT>
void sigmoid<InT, WtT, OutT>::initialize_const_params(
    ConstBufferIO &io, const std::vector<Tensor> &const_params,
    const std::map<std::string, std::any> &attr) {
  RYZENAI_LOG_TRACE("sigmoid initialize_const_params(ptr) ...");

  DD_THROW_IF((const_params.size() != 1),
              OpsFusion::dd_format("Unsupported const spec for elwdiv_qdq\n") +
  ...
  RYZENAI_LOG_TRACE("sigmoid initialize_const_params(ptr) ... DONE");
}
...
void sigmoid<InT, WtT, OutT>::initialize_const_params(
  ...
  if (const_params.size() != 1) {
    throw std::runtime_error(
        "elwdiv_qdq IPU Wrapper expect to have one constant.");
  }
  ...
  RYZENAI_LOG_TRACE("sigmoid: B_BO_SIZE:" + std::to_string(B_BO_SIZE));
...
template <typename InT, typename WtT, typename OutT>
void sigmoid<InT, WtT, OutT>::execute(std::vector<Tensor> &input,
                                      std::vector<Tensor> &output) {
  // Check the number of inputs
  if (input.size() != 1) {
    throw std::runtime_error("sigmoid IPU Wrapper expect to have one input.");
  }
  const int a_idx = 0;
  // The first data is a
  ...
  RYZENAI_LOG_TRACE("sigmoid: A_BO_SIZE:" + std::to_string(A_BO_SIZE) +
  ...
  RYZENAI_LOG_TRACE("sigmoid: a_size:" + std::to_string(a_size));
  ...
  RYZENAI_LOG_INFO(
      std::to_string(sigmoid_id_) + " " + std::to_string(a_shape_[0]) + " " +
...
template class sigmoid<uint16_t, uint16_t, uint16_t>;
} // namespace ryzenai
