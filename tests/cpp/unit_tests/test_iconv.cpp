...
int test_iconv(int CI, int YI, int XI, int CO, int YO, int XO, int KY, int KX,
  ...
  if (model_name == "m3uec" && strideH == 1) { // DWC
  ...
  *(int64_t *)(&qdq_params[qdq_c0_idx]) = c0;
  ...
  if (model_name == "m3uec" && strideH == 1) { // DWC
    ...
    if (model_name == "4x4mzdk5") { // mzdk5 4x4 design
      if (CI == 4) {
        CI_padded = 16;
      }
      if (CO == 4) {
      ...
      if (strideH == 1) { // mzdk5 3x3 stride1
        if (XI == 8) {
          std::vector<WgT> cpu_b(
          ...
          std::vector<WgT> cpu_b(
              ConvWgtTensor<WgT, C_OUT_SPLIT_CONV_mzdk5,
                            C_IN_SPLIT_CONV_mzdk5>::size(CO_padded, CI_padded,
                                                         KY, KX));
          ConvWgtTensor<WgT, C_OUT_SPLIT_CONV_mzdk5, C_IN_SPLIT_CONV_mzdk5> W(
          ...
          cpu_conv_2d<
              ActTensor<InT>,
              ConvWgtTensor<WgT, C_OUT_SPLIT_CONV_mzdk5, C_IN_SPLIT_CONV_mzdk5>,
  ...
  //                           "//bin_files//m3uec_model_files//" + mode + "_" +
  //                           ifm_shape + "_" + ofm_shape + "_data//";
  std::string data_folder = OpInterface::get_dd_base_dir() +
                            "//bin_files//mzdk5_conv_data//" + mode + "_" +
...
// strideH, int strideW
TEST(m3uec_iCONV_Testa16w8, Kernel1) {
...
TEST(m3uec_iCONV_Testa16w8, Kernel2) {
  int err_count = test_iconv<uint16_t, uint8_t, uint16_t>(
...
TEST(m3uec_iCONV_Testa16w8, Kernel3) {
  int err_count = test_iconv<uint16_t, uint8_t, uint16_t>(
...
TEST(m3uec_iCONV_Testa16w8, Kernel4) {
  int err_count = test_iconv<uint16_t, uint8_t, uint16_t>(
...
TEST(m3uec_iCONV_Testa16w8, Kernel5) {
  int err_count = test_iconv<uint16_t, uint8_t, uint16_t>(
...
TEST(m3uec_iCONV_Testa16w8, Kernel6) {
  int err_count = test_iconv<uint16_t, uint8_t, uint16_t>(
...
TEST(m3uec_iCONV_Testa16w8, Kernel7) {
  int err_count = test_iconv<uint16_t, uint8_t, uint16_t>(
...
TEST(m3uec_iCONV_Testa16w8, Kernel8) {
  int err_count = test_iconv<uint16_t, uint8_t, uint16_t>(
