...
int test_mhagprb(int St, int Di, int S, int D, bool debug = false,
  ...
  std::vector<InT> qkv_msk(qry_rows * qry_cols + key_rows * key_cols +
                           val_rows * val_cols +
                           key_rows); // only used for model data debug
  ...
  std::vector<uint8_t> wgt(1024 + bias_size * sizeof(WgT) + sizeof(lnr_lutab) +
                           sizeof(lnr_lutcd) +
                           QDQparam_size * num_qdq_nodes * sizeof(int32_t));
  ...
  *(int64_t *)(&qdq_params[(16 * 1) + qdq_c0_idx]) = smv_qdq_c0;
  ...
  std::string q_name =
      OpInterface::get_dd_base_dir() + "//bin_files//mdsqr_mha_debug//Query.bin";
  read_bin_file(q_name, reinterpret_cast<char *>(q_uint8.data()));

  std::string k_name =
      OpInterface::get_dd_base_dir() + "//bin_files//mdsqr_mha_debug//Key.bin";
  read_bin_file(k_name, reinterpret_cast<char *>(k_uint8.data()));

  std::string v_name =
      OpInterface::get_dd_base_dir() + "//bin_files//mdsqr_mha_debug//Value.bin";
  ...
  std::string mask_name =
      OpInterface::get_dd_base_dir() + "//bin_files//mdsqr_mha_debug//mask.bin";
  // std::string mask_name = OpInterface::get_dd_base_dir() +
  // "//bin_files//mdsqr_mha_debug//attention_mask_input_bf16.bin";
  ...
  std::string weight_name = OpInterface::get_dd_base_dir() +
                            "//bin_files//mdsqr_mha_debug//weights.const";
  read_bin_file(weight_name, reinterpret_cast<char *>(weight.data()));

  std::string gprb_vec64_name = OpInterface::get_dd_base_dir() +
                                "//bin_files//mdsqr_mha_debug//gprb_vec64.const";
  read_bin_file(gprb_vec64_name, reinterpret_cast<char *>(gprb_vec64.data()));

  std::string gprb_vec32_name = OpInterface::get_dd_base_dir() +
                                "//bin_files//mdsqr_mha_debug//gprb_vec32.const";
  read_bin_file(gprb_vec32_name, reinterpret_cast<char *>(gprb_vec32.data()));

  std::vector<uint8_t> bias_uint8(H * qry_rows * key_rows);
  std::string bias_name = OpInterface::get_dd_base_dir() +
                          "//bin_files//mdsqr_mha_debug//bias.const";
  ...
  std::string qdq_params_name = OpInterface::get_dd_base_dir() +
                                "//bin_files//mdsqr_mha_debug//qdq_params.const";
  ...
  std::string fld_name;
  fld_name = "//H12_v1p2_data_dd//";

  read_bin_file(OpInterface::get_dd_base_dir() + fld_name + "ifm.bin",
                reinterpret_cast<char *>(qkv_msk.data()));
  memcpy(q.data(), qkv_msk.data(), qry_rows * qry_cols * sizeof(InT));
  memcpy(k.data(),
         reinterpret_cast<char *>(qkv_msk.data()) +
             qry_rows * qry_cols * sizeof(InT),
         key_rows * key_cols * sizeof(InT));
  memcpy(v.data(),
         reinterpret_cast<char *>(qkv_msk.data()) +
             qry_rows * qry_cols * sizeof(InT) +
             key_rows * key_cols * sizeof(InT),
         val_rows * val_cols * sizeof(InT));
  memcpy(msk.data(),
         reinterpret_cast<char *>(qkv_msk.data()) +
             qry_rows * qry_cols * sizeof(InT) +
             key_rows * key_cols * sizeof(InT) +
  ...
  read_bin_file(OpInterface::get_dd_base_dir() + fld_name + "wgt.bin",
                reinterpret_cast<char *>(wgt.data()));
  ...
  for (int i = 0; i < gprb_rows * gprb_cols; ++i) {
    weight[i] = prm->proj_mat[i];
  }

  for (int i = 0; i < gprb_cols; ++i) {
    gprb_vec64[i] = prm->qdq_bias[i];
  }

  gprb_vec64[gprb_c0_scalar_idx] = prm->c0;
  gprb_vec64[qk_qdq_c0_scalar_idx] = 2396492987128832;
  gprb_vec64[smv_qdq_c0_scalar_idx] = 36806795984896;

  gprb_vec32[qdq_c1_idx] = prm->c1;
  gprb_vec32[qdq_c2_idx] = prm->c2;
  gprb_vec32[qdq_c3_idx] = prm->c3;
  gprb_vec32[qdq_SQb_idx] = prm->shift_Qb;
  gprb_vec32[qdq_Sout_idx] = prm->shift_Qout;
  gprb_vec32[qdq_Stdm_idx] = prm->res;
  gprb_vec32[gprb_act_scale_idx] = prm->act_scale.value;
  gprb_vec32[gprb_act_zero_idx] = (InT)prm->act_zero_point;
  gprb_vec32[gprb_wgt_scale_idx] = prm->wgt_scale.value;
  gprb_vec32[gprb_wgt_zero_idx] = (InT)prm->wgt_zero_point;
  for (int h = 0; h < num_heads; ++h) {
    gprb_vec32[gprb_model_a_idx + h] = prm->model_a[h].value;
  }
  gprb_vec32[gprb_model_b_idx] = prm->model_b.value;
  gprb_vec32[gprb_model_c_idx] = prm->model_c.value;
  gprb_vec32[gprb_isint16_idx] = prm->isint16;

  // memcpy(bias.data(), reinterpret_cast<char *>(wgt.data()) + 1024, bias_size
  // * sizeof(WgT));
  read_bin_file(OpInterface::get_dd_base_dir() + fld_name +
                    "GRPB_Const_Input_bias_12_512_512_mul_uint16.bin",
                reinterpret_cast<char *>(bias.data()));
  memcpy(qdq_params.data(),
         reinterpret_cast<char *>(wgt.data()) + 1024 + bias_size * sizeof(WgT) +
             sizeof(lnr_lutab) + sizeof(lnr_lutcd),
         QDQparam_size * num_qdq_nodes * sizeof(int32_t));
  ...
  std::string ref_name = OpInterface::get_dd_base_dir() +
                         "//bin_files//mdsqr_mha_debug//mhabrpb_0_ort.bin";
...
// MHA
TEST(mdsqrv1_0_MHA_Testa8w8, Kernel1) {
  int err_count = test_mhagprb<uint8_t, uint8_t, uint8_t>(
      512, 1152, 512, 768, false, "uint8", "uint8", "uint8", "mdsqr");
  EXPECT_TRUE(err_count == 0) << "Error Count = " << err_count;
}

TEST(mxpzi_MHA_Testa16w8, Kernel1) {
  int err_count = test_mhagprb<uint16_t, uint16_t, uint16_t>(
      128, 1152, 128, 768, false, "uint16", "uint16", "uint16", "mxpzi");
  EXPECT_TRUE(err_count == 0) << "Error Count = " << err_count;
}

TEST(mxganv1_1_MHA_Testa16w8, Kernel1) {
  int err_count = test_mhagprb<uint16_t, uint8_t, uint16_t>(
      512, 1152, 512, 768, false, "uint16", "uint8", "uint16", "mxgan");
  EXPECT_TRUE(err_count == 0) << "Error Count = " << err_count;
}

TEST(mdsqrv1_1_MHA_Testa8w8, Kernel1) {
  int err_count = test_mhagprb<uint8_t, uint8_t, uint8_t>(
      256, 1152, 256, 768, false, "uint8", "uint8", "uint8", "mdsqrv1.1");
  EXPECT_TRUE(err_count == 0) << "Error Count = " << err_count;
}

TEST(mxganv1_2_MHA_Testa16w8, Kernel1) {
  int err_count = test_mhagprb<uint16_t, uint16_t, uint16_t>(
      512, 1152, 512, 768, false, "uint16", "uint16", "uint16", "mxganv1.2");
  EXPECT_TRUE(err_count == 0) << "Error Count = " << err_count;
}
