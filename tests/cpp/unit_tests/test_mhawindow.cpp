...
int test_mhawindow(int M, int K, int N, bool debug = false,
  ...
  *(int64_t *)(&qdq_params[(16 * 3) + qdq_c0_idx]) = smv_qdq_c0;
  ...
  std::string fld_name;
  if (M == 1) {
    fld_name = "//bin_files//m3uec_MHAWIN//data_block3//";
  } else if (M == 4) {
    fld_name = "//bin_files//m3uec_MHAWIN//data_block2//";
  } else if (M == 16) {
    fld_name = "//bin_files//m3uec_MHAWIN//data_block1//";
  } else {
    fld_name = "//bin_files//m3uec_MHAWIN//data_block0//";
  }
  ...
  mul_val = 65535;
  // MuL DQ
  qdq_params[(16 * 0) + 0] = 33093;
  qdq_params[(16 * 0) + 1] =
      float_to_bfloat16(0.000196693057660 * (mul_val - mul_zp) * mul_scale)
          .value;
  // MuL Q
  qdq_params[(16 * 1) + 0] = 32775;
  qdq_params[(16 * 1) + 1] = float_to_bfloat16(1.0 / 0.000034432450775).value;

  // QKT
  *(int64_t *)(&qdq_params[(16 * 2) + 0]) = 854234866166560; // c0
  qdq_params[(16 * 2) + 2] = -758855583;                     // c1
  qdq_params[(16 * 2) + 3] = 1467584;                        // c2
  qdq_params[(16 * 2) + 4] = -751563525;                     // c3
  qdq_params[(16 * 2) + 5] = St_pad;
  qdq_params[(16 * 2) + 6] = St_pad;
  qdq_params[(16 * 2) + 7] = 0;  // SQb
  qdq_params[(16 * 2) + 8] = 31; // Sout
  qdq_params[(16 * 2) + 9] = 6;  // Stdm

  // SM *V
  *(int64_t *)(&qdq_params[(16 * 3) + 0]) = 17204565245952;
  qdq_params[(16 * 3) + 2] = -326594817;
  qdq_params[(16 * 3) + 3] = 1263232;
  qdq_params[(16 * 3) + 4] = 0;
  qdq_params[(16 * 3) + 5] = St_pad;
  qdq_params[(16 * 3) + 6] = val_subv_cols;
  qdq_params[(16 * 3) + 7] = 0;
  qdq_params[(16 * 3) + 8] = 29;
  qdq_params[(16 * 3) + 9] = 7;

  // DQ before SM
  qdq_params[(16 * 4) + 0] = 27170;
  qdq_params[(16 * 4) + 1] =
      float_to_bfloat16(0.000634245341644 * 1.442695041).value;

  // Q after SM
  qdq_params[(16 * 5) + 0] = 0;
  qdq_params[(16 * 5) + 1] = float_to_bfloat16(1.0 / 0.000015259021893).value;

  read_bin_file(OpInterface::get_dd_base_dir() + fld_name + "wgt.bin",
                reinterpret_cast<char *>(qdq_params.data()));
#endif
  // calculate aie_Y
  ryzenai::mhawindow mhawindow_ =
      ryzenai::mhawindow<Tqkv, WgT, OuT>(a_dtype, b_dtype, c_dtype, false);
  mhawindow_.debug(debug);
  // std::vector<size_t> a_shape = {qry_rows, qry_cols, key_rows, val_cols};
  mhawindow_.set_params(model_name, qkv_shape);

  std::vector<Tensor> const_Tensor;

  const_Tensor = {{qdq_params.data(), qdq_shape, "int32"}};
  mhawindow_.initialize_const_params(const_Tensor);
  ...
  PROFILE_THIS(mhawindow_.execute(input_Tensor, output_Tensor));
#else
  mhawindow_.execute(input_Tensor, output_Tensor);
#endif
...
// MHAWINDOW
TEST(m3uec_MHAWINDOW_Testa16w8, Kernel1) {
  int err_count = test_mhawindow<uint16_t, uint8_t, uint16_t>(
      64, 49, 384, false, "uint16", "uint8", "uint16", "m3uec");
  EXPECT_TRUE(err_count == 0) << "Error Count = " << err_count;
}

TEST(m3uec_MHAWINDOW_Testa16w8, Kernel2) {
  int err_count = test_mhawindow<uint16_t, uint8_t, uint16_t>(
      16, 49, 768, false, "uint16", "uint8", "uint16", "m3uec");
  EXPECT_TRUE(err_count == 0) << "Error Count = " << err_count;
}

TEST(m3uec_MHAWINDOW_Testa16w8, Kernel3) {
  int err_count = test_mhawindow<uint16_t, uint8_t, uint16_t>(
      4, 49, 1536, false, "uint16", "uint8", "uint16", "m3uec");
  EXPECT_TRUE(err_count == 0) << "Error Count = " << err_count;
}

TEST(m3uec_MHAWINDOW_Testa16w8, Kernel4) {
  int err_count = test_mhawindow<uint16_t, uint8_t, uint16_t>(
      1, 49, 3072, false, "uint16", "uint8", "uint16", "m3uec");
  EXPECT_TRUE(err_count == 0) << "Error Count = " << err_count;
}
