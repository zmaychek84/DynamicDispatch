...
int test_mladfsoftmax(const std::vector<size_t> shape, bool debug = false,
  ...
  const static float ifm_scale[1] = {
      0.00103351485449820756912231445312500000000};
  const static uint16_t ofm_zp[1] = {0};
  const static float ofm_scale[1] = {
      0.00001525902189669642175853803145457732171};
...
TEST(PSS_MLADFSOFTMAX, Kernel4096x4096) {
  ...
  // changing lut table into bfloat16 increased the error count, but it meet the
  // final requirement for pss/pst model
  EXPECT_TRUE(err_count == 42937) << "Error Count = " << err_count;
}
