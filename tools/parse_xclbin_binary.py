...
def write_bin_to_file(xclbin_hdrf, xclbin_srcf, data):
    ...
    with open(xclbin_srcf, "a") as src:
        ...
        src.write(f'static const std::vector<char> {variable_name}_vec = initialize_{variable_name}();\n')
...
def write_xclbin_src(out_dir, xclbin_data, quiet, xclbin_hdr):
    ...
    with open(Path(out_dir) / Path('xclbin_container.cpp'), 'w') as xclbin_src:
        ...
        xclbin_src.write('const std::vector<char>& XclbinContainer::get_xclbin_content(const std::string& name) {\n')
        ...
        xclbin_src.write('\t\tthrow std::runtime_error("Invalid xclbin string name : " + xclbin_name);\n')
        xclbin_src.write('}\n')
